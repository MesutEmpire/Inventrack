<template>
  <div class="max-w-2xl mx-auto bg-white p-16">
    <div class="p-4 rounded-lg border border-gray-200 shadow-md sm:p-6 lg:p-8">
      <form @submit.prevent="store.addNewProduct">
        <div class="grid gap-6">
          <div class="relative md:mx-6">
            <input
                type="text"
                id="food_title"
                v-model="store.productForm.title"
          ...
          <div class="relative md:mx-6">
            <input
                type="number"
                id="food_price"
                v-model="store.productForm.price"
                class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" "
                required
            />
          ...
          <div class="relative md:mx-6">
            <select
                type="number"
                id="food_category"
                v-model="store.productForm.category"
                class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" "
                required
            >
          ...
          <div class="relative md:mx-6">
            <textarea
                ...
                rows="3"
                id="food_description"
                v-model="store.productForm.description"
                class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" "
                required
            />
            ...
            <div class="flex justify-center items-center md:mx-6">
              ...
              >
                ...
                <input
                    ...
                    type="file"
                    class="hidden"
                    @input="event => store.productForm.img = event.target.files[0]">
                <div
                    v-if="store.productForm.img != ''"
                    class="aspect-w-2 aspect-h-3 rounded-lg bg-white overflow-hidden sm:col-span-4 lg:col-span-5"
                >
                  <img
                      :src="store.productForm.img"
                      :alt="store.productForm.title"
